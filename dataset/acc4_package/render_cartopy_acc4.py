#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ACC-4 seasonal belts (Cartopy, Robinson/Mercator) — COOL & WARM maps, with PDF-tone colors and --override."""
...
def build_palette(pal_csv, labels4, overrides):
    pal = pd.read_csv(pal_csv).sort_values("id")
    maxid = pal["id"].max()
    lut = np.zeros((maxid+1,3), dtype=np.uint8)
    for _,r in pal.iterrows():
        lut[int(r["id"]),:] = [int(r["R"]),int(r["G"]),int(r["B"])]
    if overrides:
        lab2id = {lab:i+1 for i,lab in enumerate(labels4)}
...
def main():
    ...
    ap.add_argument("--npz", default="acc4_seasonal.npz")
    ap.add_argument("--palette", default="palette_4_pdfcolors.csv")
    ap.add_argument("--out_cool", default="acc4_ONDJFM_robinson.png")
    ap.add_argument("--out_warm", default="acc4_AMJJAS_robinson.png")
    ...
    cool4=Z["cool4_id"].astype(np.uint8); warm4=Z["warm4_id"].astype(np.uint8); labels4=[s for s in Z["labels4"]]
    ...
    lut=build_palette(args.palette, labels4, overrides)
    proj = ccrs.Robinson() if args.proj=="robinson" else ccrs.Mercator()

    fig=plt.figure(figsize=(12,6), dpi=220); ax=plt.axes(projection=proj); draw(ax,cool4,lut,f"4 airmass zones (ONDJFM), Shimabukuro et al. (2023) ({args.proj.title()})"); fig.savefig(args.out_cool,bbox_inches="tight"); plt.close(fig)
    fig=plt.figure(figsize=(12,6), dpi=220); ax=plt.axes(projection=proj); draw(ax,warm4,lut,f"4 airmass zones (AMJJAS), Shimabukuro et al. (2023) ({args.proj.title()})"); fig.savefig(args.out_warm,bbox_inches="tight"); plt.close(fig)
